...
<?php
  ...
  // Map existing loading_product_total data to variations
  // Match by variation order (index) for each product
  foreach ($products_by_id as $pop_id => &$data) {
      if (isset($loading_totals_by_parent[$pop_id])) {
          $lt_data = $loading_totals_by_parent[$pop_id];
          // Sort by id to maintain order
          usort($lt_data, function($a, $b) {
              return $a['id'] - $b['id'];
          });
          
          // Map loading_total data to variations by index
          foreach ($data['variations'] as $var_index => &$var) {
              if (isset($lt_data[$var_index])) {
                  $var['loading_total'] = $lt_data[$var_index];
              }
          }
          // If no variations, use first loading_total for main row
          if (empty($data['variations']) && !empty($lt_data)) {
              $data['product']['loading_total'] = $lt_data[0];
          }
      }
  }
  ...
  $invoice_date_map = [];
  $invoice_terms_map = [];
  $invoice_price_terms_map = [];
  foreach ($products_by_id as $data) {
      $product = $data['product'];
    //   echo json_encode($product);
      if (!empty($product['invoice_no'])) {
          if (!empty($product['invoice_supplier_id'])) {
              $invoice_supplier_map[$product['invoice_no']] = $product['invoice_supplier_id'];
          }
          // Get invoice info, date, terms, and price terms (use first product's values for each invoice)
          if (!isset($invoice_info_map[$product['invoice_no']]) && !empty($product['invoice'])) {
              $invoice_info_map[$product['invoice_no']] = $product['invoice'];
          }
          if (!isset($invoice_date_map[$product['invoice_no']]) && !empty($product['invoice_date'])) {
              $invoice_date_map[$product['invoice_no']] = $product['invoice_date'];
          }
          if (!isset($invoice_terms_map[$product['invoice_no']]) && !empty($product['invoice_terms'])) {
              $invoice_terms_map[$product['invoice_no']] = $product['invoice_terms'];
          }
          if (!isset($invoice_price_terms_map[$product['invoice_no']]) && !empty($product['invoice_price_terms'])) {
              $invoice_price_terms_map[$product['invoice_no']] = $product['invoice_price_terms'];
          }
      }
  }
?>
                                ...
                                <tbody class="loading_list_tbody">
                                    ...
                                    <tr id="loading_row_<?php echo $product['id']; ?>" class="main-product-row" data-row-id="<?php echo $product['id']; ?>" data-product-id="<?php echo $product['product_id']; ?>">
                                        ...
                                        <td rowspan="<?php echo $rowspan; ?>">
                                            <select class="form-control form-control-sm invoice-select" 
                                                ...
                                                <?php 
                                                $existing_invoice = isset($product['invoice_no']) ? intval($product['invoice_no']) : 0;
                                                for($i = 1; $i <= 5; $i++): 
                                                    $selected = ($existing_invoice == $i) ? 'selected' : '';
                                                ?>
                                        ...
                                        <td rowspan="<?php echo $rowspan; ?>">
                                            <input type="number" min="0" step="1" class="form-control form-control-sm loading-qty" 
                                                ...
                                                value="<?php echo isset($product['loading_qty']) ? $product['loading_qty'] : ''; ?>"
                                        ...
                                        <td rowspan="<?php echo $rowspan; ?>">
                                            <input type="number" min="0" step="0.01" class="form-control form-control-sm unit-price-rmb" 
                                                ...
                                                value="<?php echo isset($product['unit_price_rmb']) ? number_format($product['unit_price_rmb'], 2, '.', '') : ''; ?>"
                                        ...
                                        <td rowspan="<?php echo $rowspan; ?>">
                                            <input type="number" min="0" step="1" class="form-control form-control-sm official-ci-qty" 
                                                ...
                                                value="<?php echo isset($product['official_ci_qty']) ? $product['official_ci_qty'] : ''; ?>"
                                                onkeyup="calculateRow(<?php echo $product['id']; ?>);"
                                                oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                onchange="calculateRow(<?php echo $product['id']; ?>);">
                                        </td>
                                        ...
                                        <td rowspan="<?php echo $rowspan; ?>">
                                            <input type="number" step="any" class="form-control form-control-sm black-qty" 
                                                id="black_qty_<?php echo $product['id']; ?>" name="black_qty[<?php echo $product['id']; ?>]" 
                                                value="<?php echo isset($product['black_qty']) ? $product['black_qty'] : ''; ?>" readonly>
                                        </td>
                                        <td rowspan="<?php echo $rowspan; ?>">
                                            <input type="number" step="0.01" class="form-control form-control-sm total-amount-rmb" 
                                                id="total_amount_rmb_<?php echo $product['id']; ?>" name="total_amount_rmb[<?php echo $product['id']; ?>]" 
                                                value="<?php echo isset($product['total_amount_rmb']) ? number_format($product['total_amount_rmb'], 2, '.', '') : ''; ?>" readonly>
                                        </td>
                                        <td rowspan="<?php echo $rowspan; ?>">
                                            <input type="number" step="0.01" class="form-control form-control-sm official-ci-unit-price-usd" 
                                                ...
                                                value="<?php echo isset($product['official_ci_unit_price_usd']) ? number_format($product['official_ci_unit_price_usd'], 2, '.', '') : number_format($usd_rate, 2, '.', ''); ?>" name="official_ci_unit_price_usd[<?php echo $product['id']; ?>]">
                                        </td>
                                        <td rowspan="<?php echo $rowspan; ?>">
                                        ...
                                        // Get existing loading_total data for this variation
                                        $lt_data = null;
                                        if (!empty($variations) && isset($variations[0]['loading_total'])) {
                                            $lt_data = $variations[0]['loading_total'];
                                        } elseif (empty($variations) && isset($product['loading_total'])) {
                                            $lt_data = $product['loading_total'];
                                        }
                                        
                                        // Use existing values if available, otherwise use defaults
                                        $pkg_ctn_val = $lt_data ? number_format($lt_data['pkg_ctn'], 2, '.', '') : $product['loading_qty'];
                                        $nw_kg_val = $lt_data ? number_format($lt_data['nw_kg'], 2, '.', '') : number_format($net_weight, 2, '.', '');
                                        $gw_kg_val = $lt_data ? number_format($lt_data['gw_kg'], 2, '.', '') : number_format($gross_weight, 2, '.', '');
                                        $length_val = $lt_data ? number_format($lt_data['length'], 2, '.', '') : number_format($length, 2, '.', '');
                                        $width_val = $lt_data ? number_format($lt_data['width'], 2, '.', '') : number_format($width, 2, '.', '');
                                        $height_val = $lt_data ? number_format($lt_data['height'], 2, '.', '') : number_format($height, 2, '.', '');
                                        $total_nw_val = $lt_data ? number_format($lt_data['total_nw_kg'], 2, '.', '') : '';
                                        $total_gw_val = $lt_data ? number_format($lt_data['total_gw_kg'], 2, '.', '') : '';
                                        $total_cbm_val = $lt_data ? number_format($lt_data['total_cbm_value'], 6, '.', '') : '';
                                        ?>
                                        ...
                                        <td class="metric-cell">
                                            <input type="number" step="0.01" class="form-control form-control-sm total-nw" 
                                                ...
                                                value="<?php echo $total_nw_val; ?>" readonly>
                                        </td>
                                        <td class="metric-cell">
                                        ...
                                        <td class="metric-cell">
                                            <input type="number" step="0.01" class="form-control form-control-sm total-gw" 
                                                ...
                                                value="<?php echo $total_gw_val; ?>" readonly>
                                        </td>
                                        <td class="metric-cell">
                                    ...
                                    <?php if (count($variations) > 1): ?>
                                        ...
                                        foreach (array_slice($variations, 1) as $var): 
                                            // Get existing loading_total data for this variation
                                            $var_lt_data = isset($var['loading_total']) ? $var['loading_total'] : null;
                                            
                                            // Use existing values if available, otherwise use defaults
                                            $var_pkg_ctn_val = $var_lt_data ? number_format($var_lt_data['pkg_ctn'], 2, '.', '') : $product['loading_qty'];
                                            $var_nw_kg_val = $var_lt_data ? number_format($var_lt_data['nw_kg'], 2, '.', '') : number_format($var['net_weight'], 2, '.', '');
                                            $var_gw_kg_val = $var_lt_data ? number_format($var_lt_data['gw_kg'], 2, '.', '') : number_format($var['gross_weight'], 2, '.', '');
                                            $var_length_val = $var_lt_data ? number_format($var_lt_data['length'], 2, '.', '') : number_format($var['length'], 2, '.', '');
                                            $var_width_val = $var_lt_data ? number_format($var_lt_data['width'], 2, '.', '') : number_format($var['width'], 2, '.', '');
                                            $var_height_val = $var_lt_data ? number_format($var_lt_data['height'], 2, '.', '') : number_format($var['height'], 2, '.', '');
                                            $var_total_nw_val = $var_lt_data ? number_format($var_lt_data['total_nw_kg'], 2, '.', '') : '';
                                            $var_total_gw_val = $var_lt_data ? number_format($var_lt_data['total_gw_kg'], 2, '.', '') : '';
                                            $var_total_cbm_val = $var_lt_data ? number_format($var_lt_data['total_cbm_value'], 6, '.', '') : '';
                                        ?>
...
var invoiceSupplierMap = {};
<?php foreach($invoice_supplier_map as $invoice_no => $supplier_id): ?>
invoiceSupplierMap[<?php echo $invoice_no; ?>] = <?php echo $supplier_id; ?>;
<?php endforeach; ?>

// Invoice info, date, and terms mappings for pre-selection
var invoiceInfoMap = {};
<?php foreach($invoice_info_map as $invoice_no => $invoice_info): ?>
invoiceInfoMap[<?php echo $invoice_no; ?>] = <?php echo json_encode($invoice_info); ?>;
<?php endforeach; ?>

var invoiceDateMap = {};
<?php foreach($invoice_date_map as $invoice_no => $invoice_date): ?>
invoiceDateMap[<?php echo $invoice_no; ?>] = <?php echo json_encode($invoice_date); ?>;
<?php endforeach; ?>

var invoiceTermsMap = {};
<?php foreach($invoice_terms_map as $invoice_no => $invoice_terms): ?>
invoiceTermsMap[<?php echo $invoice_no; ?>] = <?php echo json_encode($invoice_terms); ?>;
<?php endforeach; ?>

var invoicePriceTermsMap = {};
<?php foreach($invoice_price_terms_map as $invoice_no => $invoice_price_terms): ?>
invoicePriceTermsMap[<?php echo $invoice_no; ?>] = <?php echo json_encode($invoice_price_terms); ?>;
<?php endforeach; ?>
...
function appendLoadingListProductRow($section, productData) {
    ...
    for (var i = 0; i < rowSpan; i++) {
        ...
        var netWeight = toNumber(variation.net_weight || productData.net_weight || 0);
        var grossWeight = toNumber(variation.gross_weight || productData.gross_weight || 0);
        var length = toNumber(variation.length || productData.length || 0);
        var width = toNumber(variation.width || productData.width || 0);
        var height = toNumber(variation.height || productData.height || 0);

        var rowClass = i === 0 ? 'main-product-row' : 'variation-row';
        var rowId = i === 0 ? 'loading_row_' + rowKey : '';
        var rowAttrs =
            i === 0
...
function updateInvoiceSuppliers() {
    ...
    if (uniqueInvoices.length > 0) {
        ...
        uniqueInvoices.forEach(function(invoiceNo) {
            ...
            var unfilteredDate = invoiceDateMap[invoiceNo] || currentDate;
            var dateParts = unfilteredDate.split(' ');

            var existingInvoiceInfo = invoiceInfoMap[invoiceNo] || '';
            var existingInvoiceDate = dateParts[0];
            var existingInvoiceTerms = invoiceTermsMap[invoiceNo] || '';
            var existingInvoicePriceTerms = invoicePriceTermsMap[invoiceNo] || '';
            ...
            var dropdownHtml = '<div class="col-md-6">' +
                ...
                'id="invoice_' + invoiceNo + '" value="' + escapeAttr(existingInvoiceInfo) + '" placeholder="Enter invoice information">' +
                '</div>' +
                '<div class="invoice-field-group">' +
                '<label class="invoice-supplier-label">' +
                ...
                'id="invoice_date_' + invoiceNo + '" value="' + escapeAttr(existingInvoiceDate) + '" required>' +
                '</div>' +
                '<div class="invoice-field-group">' +
                '<label class="invoice-supplier-label">' +
                ...
                'id="invoice_terms_' + invoiceNo + '" placeholder="Enter invoice terms">' + escapeHtml(existingInvoiceTerms) + '</textarea>' +
                '</div>' +
                '<div class="invoice-field-group">' +
                '<label class="invoice-supplier-label">' +
                ...
                'id="invoice_price_terms_' + invoiceNo + '" value="' + escapeAttr(existingInvoicePriceTerms) + '" placeholder="Enter price term">' +
                '</div>' +
                '</div>' +
                '</div>';
            ...
            if (invoiceSupplierMap[invoiceNo]) {
                $('#invoice_supplier_' + invoiceNo).val(invoiceSupplierMap[invoiceNo]);
            }
        });
    } else {
