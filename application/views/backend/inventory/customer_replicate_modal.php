...
<div class="row">
  <div class="col-12">
      <?php echo form_open('inventory/customer/replicate_post', ['id' => 'customer_replicate_form', 'onsubmit' => 'return submitReplicateForm(event);']); ?>
      ...
      <div class="row">
        ...
        <div class="col-12">
          <button type="submit" id="replicate_submit_btn" class="btn btn-primary waves-effect waves-float waves-light">
            <i class="fa fa-refresh"></i> Replicate Customer
          </button>
...
function submitReplicateForm(event) {
  ...
  // Get form data
  var formData = $('#customer_replicate_form').serialize();
  var formUrl = $('#customer_replicate_form').attr('action');
  ...
  $.ajax({
    ...
    success: function(res) {
      if (res.status == '200' || res.status == 200) {
        ...
        Swal.fire({
          title: "Success!",
          text: res.message || "Customer replicated successfully",
          icon: "success",
          customClass: {
            confirmButton: "btn btn-primary"
          },
        ...
        Swal.fire({
          title: "Error!",
          text: res.message || "An error occurred while replicating customer",
          icon: "error",
          customClass: {
            confirmButton: "btn btn-primary"
          },
