  ...
  <div class="col-12">
    <div class="card">
      <div class="card-body py-1 my-0">
        ...
        <?php echo form_open('inventory/company_sales/add_post', ['class' => 'add-ajax-redirect-form','onsubmit' => 'return validateForm() && checkForm(this);']);?>
        <div class="row">
          ...
          <div class="col-12 col-sm-3 mb-1">
            <div class="form-group">
              <label>Reference Order No </label>
          ...
          <div class="col-12 col-sm-3 mb-1">
            <label class="form-label" for="customer_id">Company <span class="required">*</span></label>
...
function get_details_by_product(product_id, index) {
  ...
  $.ajax({
    type: "POST",
    url: "<?php echo base_url()?>inventory/get_company_sales_product_details",
    ...
    success: function(res) {
      if(res.status == 200) {
        ...
        $('#bill_amount_' + index).val(res.off_sale_price ? parseFloat(res.off_sale_price).toFixed(2) : '0.00');
        calculate_amt(index);
      } else {
        alert(res.message);
      }
...
$(document).ready(function ($) {
  ...
  // Auto-open Select2 dropdown on focus
  ...
  $('#customer_id').on('change', function() {
    ...
    if(company_id) {
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>inventory/get_company_details_ajax",
...
function rollup_product_totals(index) {
  ...
  $('.batch-row-' + index).each(function() {
    ...
    total_amt += parseFloat($(this).find('.batch_total_amount').val()) || 0;
  ...
  $('#total_amount_' + index).val(total_amt.toFixed(2));
  $('#bill_total_' + index).val(total_bill_amt.toFixed(2));
  ...
  if (total_allocated > 0) {
    $('#master_amount_' + index).val((total_amt / total_allocated).toFixed(2));
...
function calculate_batch_amt(element, index) {
  ...
  var black_amt_unit = black_qty > 0 ? (black_total_amt / black_qty) : (rate - bill_amt);
...
function addBatch(index) {
  ...
  var batch_row = `
    <tr class="batch-row batch-row-${index}">
      ...
      <td>
        <input type="number" step="any" class="form-control batch_rate text-center" name="batch_rate[${index}][]" id="batch_rate_${index}_${batch_index}" readonly>
      </td>
      ...
      <td>
        <input type="number" step="any" class="form-control batch_bill_amount text-center" name="batch_bill_amount[${index}][]" id="batch_bill_amount_${index}_${batch_index}" onkeyup="calculate_batch_amt(this, '${index}')">
      </td>
      <td style="min-width: 170px;">
      ...
      <td>
        <input type="number" step="any" class="form-control batch_bill_total text-center" name="batch_bill_total[${index}][]" id="batch_bill_total_${index}_${batch_index}" readonly>
      </td>
      <td>
...
function getBatchDetails(element, index) {
  ...
  if (batch_id == '') {
    ...
    row.find('.batch_bill_amount').val(0);
  ...
  $.ajax({
    type: "POST",
    url: "<?php echo base_url() ?>inventory/get_company_sales_batch_details",
    data: {
      batch_id: batch_id
    },
    dataType: 'json',
    success: function(res) {
      ...
      // Auto populate actual_cost_with_exp into batch_rate (readonly)
      var cost_with_exp = res.actual_cost_with_exp ? parseFloat(res.actual_cost_with_exp).toFixed(2) : '0.00';
      row.find('.batch_rate').val(cost_with_exp);
      
      // Auto populate official amount from off_sale_price
      var off_price = res.off_sale_price ? parseFloat(res.off_sale_price).toFixed(2) : '0.00';
      row.find('.batch_bill_amount').val(off_price);
      
      var main_gst = $('#gst_' + index).val() || (res.gst ? parseFloat(res.gst) : 0);
