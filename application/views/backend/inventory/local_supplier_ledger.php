...
$supplier_id = (int) ($id ?? ($supplier['id'] ?? 0));
$base_ledger_url = base_url('inventory/local-supplier/ledger/' . $supplier_id);
...
<style>
  ...
  .supplier-info-col { min-width: 200px; }
...
<div class="bg-white supplier-card-shell mb-3">
  <div class="d-flex align-items-center justify-content-between px-3 py-2 card-soft-header flex-wrap gap-2">
    <div>
      <div class="text-uppercase fw-semibold fs-10 track-1 mb-1">Local Supplier Ledger</div>
      <div class="fw-semibold supplier-main-text fs-15"><?= html_escape($supplier['name'] ?? '—') ?></div>
    </div>
    <div class="d-flex align-items-center flex-wrap gap-2">
      <a href="<?= base_url('inventory/local-supplier') ?>" class="btn btn-sm btn-outline-secondary fs-11">Back</a>
    </div>
  </div>
  <div class="d-flex flex-wrap">
    <div class="flex-fill px-3 py-2 supplier-info-divider supplier-info-col">
      <div class="text-uppercase fw-semibold fs-10 track-1 mb-2">Address</div>
      <?php if (!empty($supplier['gst_name'])): ?>
        <div class="fs-13 fw-semibold supplier-main-text mb-1"><?= html_escape($supplier['gst_name']) ?></div>
      <?php endif; ?>
      ...
      <?php if (!empty($supplier['country_name'])): ?>
        <div class="fs-11 text-muted"><?= html_escape($supplier['country_name']) ?></div>
      <?php endif; ?>
    </div>
    <div class="flex-fill px-3 py-2 supplier-info-col">
      ...
      <table class="w-100 key-info-table fs-12">
        ...
        <?php endif; ?>
        <tr>
          <td class="text-secondary fw-medium">Opening (INR)</td>
          <td class="supplier-main-text fw-semibold">₹ <?= number_format((float)($supplier['outstanding_inr'] ?? 0), 2) ?></td>
        </tr>
      </table>
    </div>
  </div>
...
$s_pur  = $summary['purchases'] ?? ['total' => 0, 'white' => 0, 'black' => 0];
$s_pay  = $summary['payments'] ?? ['total' => 0, 'official' => 0, 'unofficial' => 0];
...
<div class="ledger-summary-grid mb-2">
  ...
  <div class="ledger-summary-card">
    ...
    <div class="val">₹ <?= number_format((float)$s_pur['total'], 2) ?></div>
    <div class="sub">White: ₹ <?= number_format((float)$s_pur['white'], 2) ?><br>Black: ₹ <?= number_format((float)$s_pur['black'], 2) ?></div>
  </div>
  <div class="ledger-summary-card">
    <div class="lbl">Payments</div>
    <div class="val">₹ <?= number_format((float)$s_pay['total'], 2) ?></div>
    <div class="sub">Official: ₹ <?= number_format((float)$s_pay['official'], 2) ?><br>Unofficial: ₹ <?= number_format((float)$s_pay['unofficial'], 2) ?></div>
  </div>
  <div class="ledger-summary-card">
...
<div class="bg-white ledger-card-shell" id="ledgerShell" data-mode="all">
  ...
  $row_class = function ($row) {
    ...
    if ($t === 'purchase') return 'ledger-row-purchase';
    if ($t === 'adjustment') {
        ...
        <?php else: ?>
          <?php foreach ($order_rows as $row): ?>
            ...
            <tr class="ledger-row <?= $row_class($row) ?>">
              ...
              <td class="col-base fw-semibold supplier-soft-text"><?= html_escape($row['particulars'] ?? '') ?></td>
      ...
      <tbody>
        ...
        <?php else: ?>
          <?php foreach ($product_rows as $row): ?>
            ...
            <tr class="ledger-row <?= $row_class($row) ?>">
              <td class="col-base text-secondary"><?= html_escape($date_disp) ?></td>
              <td class="col-base fw-semibold supplier-soft-text particulars"><?= html_escape($row['particulars'] ?? '') ?></td>
              <td class="col-base"><?= html_escape($row['vch_type'] ?? '') ?></td>
              <td class="col-base"><?= html_escape($row['vch_no'] ?? '') ?></td>
