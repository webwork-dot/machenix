...
class File_model extends CI_Model
{
	...
	public function upload_image()
	{
    	...
    	if (!empty($temp_path)) {
    		...
    		$image = $this->upload_model->img_upload($temp_path, $directory);
			$data = array(
		...
		if (!empty($modesy_images)) {
			foreach ($modesy_images as $modesy_image) {
				if (!empty($modesy_image)) {
					...
					copy(FCPATH . "uploads/temp/" . $modesy_image->img_default, $directory . $modesy_image->img_default);
				// 	copy(base_path() . "uploads/temp/" . $modesy_image->img_default, "../".$directory . $modesy_image->img_default);
					 				
					delete_file_from_server(FCPATH ."uploads/temp/" . $modesy_image->img_default);
				// 	delete_file_from_server(base_path() ."uploads/temp/" . $modesy_image->img_default);

					//add to database
					$data = array(
						'product_id' => $product_id,
						'image' => $directory.$modesy_image->img_default,
	...
	{
		$this->db->where('product_id', $product_id);
		$this->db->where('variation_id', 0);
		$this->db->where('type', 'image');
		$this->db->order_by('product_images.is_main', 'DESC');
		$query = $this->db->get('product_images');
		//echo $this->db->last_query();exit();
		return $query->result();
	}
	
	public function get_product_video_uncached($product_id) {
		$this->db->where('product_id', $product_id);
		$this->db->where('variation_id', 0);
	...
	public function delete_product_image($image_id)
	{
		...
		if (!empty($image)) {
			if ($image->storage == "aws_s3") {
				...
				$this->aws_model->delete_product_object($image->image);
				$this->aws_model->delete_product_object($image->image_big);
				$this->aws_model->delete_product_object($image->image_small);
			} else {
				delete_file_from_server($image->image);
				// delete_file_from_server("../uploads/products/" . $image->image_big);
				// delete_file_from_server("../uploads/products/" . $image->image_small);
			}
