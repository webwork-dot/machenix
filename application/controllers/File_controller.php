...
class File_controller extends CI_Controller
{
		...
		if (!empty($modesy_images)) {
			foreach ($modesy_images as $modesy_image) {
				if ($modesy_image->file_id == $file_id) {
					$main = '<a href="javascript:void(0)" class="badge badge-secondary badge-is-image-main btn-set-image-main-session" data-file-id="' . $modesy_image->file_id . '">main</a>';
					echo '<img src="' . base_url() . "uploads/temp/" . $modesy_image->img_default . '" alt="">' .
						'<a href="javascript:void(0)" class="btn-img-delete btn-delete-product-img-session" data-file-id="' . $modesy_image->file_id . '"><i class="fa fa-times-circle"></i></a>';
					break;
				}
			}
		}
	...
	public function get_uploaded_image()
	{
		...
		if (!empty($product_image)) {
			$main = '<a href="javascript:void(0)" class="badge badge-secondary badge-is-image-main btn-set-image-main" data-image-id="' . $product_image->id . '" data-product-id="' . $product_image->product_id . '">main</a>';
			echo '<img src="'.get_product_image_url($product_image, 'image') .'">' .
				'<a href="javascript:void(0)" class="btn-img-delete btn-delete-product-img" data-file-id="' . $product_image->id . '"><i class="fa fa-times-circle"></i></a>';
		}
	}
